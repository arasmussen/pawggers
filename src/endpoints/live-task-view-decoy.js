...
const PageJS = `
const socket = io();
socket.on('update-decoy-task-view', (message) => {
  // get current scroll
...
module.exports = function(request, response, server) {
  console.log('[' + new Date().toISOString() + '] /api/live-task-view-decoy');

  // start socket server
  socketServer.start(server);

  // generate body
  const body = generateDecoyTaskBody();
